...
- **vitest integration 은 서로 경합했다** (2026-08-20 원인 확정·수정 완료). 파일을 병렬로 돌리면
  통합 테스트들이 로컬 Supabase 하나를 동시에 두드려 5초 타임아웃으로 매번 다른 한 건이 깨졌다
  (푸시 훅까지 막았다). `vitest.config.ts` 를 unit(병렬)·integration(직렬) 두 프로젝트로 갈라 해결.
  그래도 산발적으로 깨지면 dev 서버가 같은 DB 를 쓰고 있는지 먼저 보라
