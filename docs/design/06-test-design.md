...
- [ ] 각 화면 주 CTA 1개 확인 (T8 실측 정합화): 목록="새 여행 만들기" · 캔버스 기본=강조 CTA 0(주 행동이 검색 입력이므로 허용) · 미리보기 카드/시트="메모 저장하기"(일정 배치는 리스트 행의 "일정에 넣기" 소관) · 편집기 펼침 시 해당 편집기 저장 버튼 1개 · 공유 뷰=CTA 없음(읽기전용)
- [ ] 진입 즉시 모달 0건 (T-04)
