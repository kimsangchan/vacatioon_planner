...
- [x] T10-20 교체 후보 목록 (결정 #53, 2026-08-21) — `lib/geo/distance.ts`(하버사인)
  + `lib/timeline/swap.ts`. 같은 카테고리 먼저 → 가까운 순 → 별 많은 순 → 이름순.
  자기 자신 제외, 배치된 곳은 "3일차 2번째에 있어요" 라벨. vitest 15
- [x] T10-21 자리는 두고 장소만 교체 (결정 #53, 2026-08-21) — `StopPatch` 에 `place_id`.
  **마이그레이션 0건** — `updateStop` 이 이미 PostgREST 직접 PATCH 이고 RLS 가 day 소유권만 본다.
  integration 1(순서·시각·확정 불변 + 플랜 A 보관함 복귀) · pgTAP 1(교차 테넌트 0행)
- [x] T10-22 타임라인 행에서 바꾸기 (결정 #53, 2026-08-21) — 행 ⋯ → '다른 곳으로 바꾸기' →
  인라인 후보 목록(hover 시 지도 핀 강조) → 즉시 교체 + '되돌리기'. vitest 7
- [x] T10-23 카드에서 바꾸기 — 모바일 현장 3탭 (결정 #53, 2026-08-21) — 핀 탭 → '다른 곳으로
  바꾸기' → 후보 탭. vitest 5 + **E2E(390×844)**: 교체·플랜 A 보관함 복귀 실측
- [x] T10-24 손가락 크기 (사용자 지적, 2026-08-21) — 별표가 24px 다섯이 붙어 있어 못 눌렀다.
  `StarRating` 에 size(touch 44px 기본 / compact) — PC 말풍선만 compact(#52).
  공유 화면은 별을 이름 아래 제 줄로 내린다(가로로 자리가 없다). 확정 체크 히트영역 42→46px.
  vitest 5 + **E2E 실측 44×44px**
- [x] T10-25 검색 버튼 · 지도 부근 정렬 (사용자 지적, 2026-08-21) — 입력 오른쪽에 '검색'
  (엔터·모바일 검색키 = submit, 자동 300ms 는 그대로). 결과를 **검색을 건 순간의 지도 중심**
  기준 거리순으로 세우고 거리를 함께 낸다. `MapProvider.viewCenter()` 신설(bounds 중점). vitest 6.
  ⚠️ **근본 해결은 아니다** — 네이버 지역검색은 좌표로 걸러 주지 않아 받은 5건 안에서만 세운다
- [x] T10-26 전화번호는 손으로 · 패널 별 넘침 (결정 #57, 2026-08-21) — **#52 정정**:
  네이버 `telephone` 은 항상 빈 문자열(실호출 10건 전부). 카드 편집에 전화번호 칸 +
  `updatePlacePhone`. **#54 회귀 수정**: 데스크톱 패널도 같은 시트라 별이 14px 넘쳤다 —
  `starSize` 로 마우스 자리는 compact. vitest 8 + E2E 넘침 단언(빨간불 확인 후 고침)
- [x] T10-27 영업시간 직접 입력 (결정 #58, 2026-08-21) — `0015` text 컬럼 + 카드 빠른 입력
  (`매일·평일/주말·24시간·예약제`) + 여러 줄 자유 수정. 읽기·공유 화면에서 줄바꿈 보존
- [x] T10-28 보관함 분류·검색 (결정 #59, 2026-08-21) — 전체·식당·숙박·스팟 개수 필터,
  이름·주소 검색, 0건 초기화, 핀에서 들어오면 숨은 필터 자동 해제
- [x] T10-29 모바일 지도 장소명 (결정 #60, 2026-08-21) — 줌 14부터 SDK Marker 안에 이름,
  선택 핀은 항상 표시. 같은 marker position·anchor 유지 + HTML 이스케이프 + 리스너 누수 방지
- [x] T10-30 공유 최신 정보 (결정 #61, 2026-08-21) — `0016` live projection + 최초·수동·focus·
  visibility 갱신. 중복 요청 방지, 영업시간 공개, 개인 메모·비용·사진·예약정보 비공개
- [x] T10-31 누른 것을 버리지 않는다 (2026-08-21) — `PreviewCard.run()` 이 `if (busy) return` 으로
  **조용히 반환**해서, 저장하기 직후 고른 사진이 사라졌다(여정 1 E2E 가 잡았다). 파일 입력까지
  비운 뒤라 재시도도 막혔다. 버리는 대신 **줄을 세운다**(누른 순서대로). 파일 입력의 `disabled={busy}`
  도 뗀다 — 잠그면 줄에 서지도 못한다. vitest 2 + E2E 정정
  (그 E2E 는 **거짓 초록불**이었다: 편집을 연 채 `toContainText` 로 재서 타이핑한 값이 잡혔다.
  읽기 화면으로 옮겼다)
...
- [x] T10-6 검색 결과 5건 상한 — **닫음(구현 아님, 결정 #56 · 2026-08-21)**. 실호출 재확인:
  `display=30` 도 `"display":5,"total":5` · "카페"도 total 5 · `start=6` 무시 · `sort=comment`도 5.
  네이버로는 못 늘린다. 카카오 Local 전환은 **약관으로 기각**(타사 지도에 타사 POI — 01-recon §약관).
  대신 받은 5건을 지도 중심 거리순으로 세우고 거리를 표기한다(T10-25)
